"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var AdvertisementForm = (function () {
    function AdvertisementForm() {
        this.counter = 0;
        this.defaultName = 'Padmaja Huddar';
        this.title = 'Advertisement Form';
        //advertisements: Array<any>=[];
        this.childEvent = new core_1.EventEmitter();
        this.categories = ['Furniture', 'Hardware', 'Mobile'];
    }
    AdvertisementForm.prototype.submitAdvertisement = function (titleRef, nameRef, categoryRef, descripRef) {
        this.counter++;
        var obj = { id: this.counter, title: titleRef, name: nameRef, category: categoryRef, description: descripRef };
        // this.advertisements.push({'title':titleRef,'name':nameRef,'category':categoryRef,'description':descripRef});
        this.childEvent.emit(obj);
    };
    return AdvertisementForm;
}());
AdvertisementForm = __decorate([
    core_1.Component({
        selector: 'AdvertisementForm',
        templateUrl: './AdvertisementForm.html',
        outputs: ['childEvent']
    }),
    __metadata("design:paramtypes", [])
], AdvertisementForm);
exports.AdvertisementForm = AdvertisementForm;
//# sourceMappingURL=advertisementForm.component.js.map